...
export class BGPViewScanner implements ScannerProvider {
  ...
  async scan(indicator: string, type: IndicatorType, _apiKey: string): Promise<EnrichmentResult> {
    const res = await proxyFetch(`https://api.bgpview.io/ip/${indicator}`)
    ...
    prefixes.slice(0, 3).forEach(p => {
      ...
      if (asnNum) relationships.push({
        targetLabel: `AS${asnNum}${asnName ? ` (${asnName})` : ''}`,
        targetType: 'Infrastructure', edgeType: 'hosts',
      })
      if (prefix) relationships.push({ targetLabel: prefix, targetType: 'Infrastructure', edgeType: 'related_to' })
    })

    return {
      provider: 'BGPView', indicatorType: type, indicator,
      riskScore: 0, confidence: 0.7, tags: [...new Set(tags)],
      relationships, rawData: data as Record<string, unknown>, timestamp: Date.now(),
    }
  }
}
