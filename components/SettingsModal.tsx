...
const KEY_GROUPS = [
  {
    ...
    keys: [
      ...
      { id: 'phishtank',   label: 'PhishTank',         url: 'https://www.phishtank.com/api_info.php',        required: false, free: 'Free (opt)'  },
    ],
  },
  {
    label: 'No Key Required (auto)',
    keys: [] as typeof KEY_GROUPS[0]['keys'],
    note: 'MalwareBazaar · WHOIS · BGPView · crt.sh · TorCheck · ip-api.com · URLhaus run automatically',
  },
  {
    label: 'AI Summary — 100% Free',
    keys: [
      { id: 'gemini', label: 'Google Gemini 2.0 Flash', url: 'https://aistudio.google.com/apikey',  required: false, free: '1500/day' },
      { id: 'groq',   label: 'Groq (Llama 3.3 70B)',    url: 'https://console.groq.com/keys',       required: false, free: '14k/day' },
    ],
  },
]
      ...
      <div className="bg-surface border border-border rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-glow flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 flex flex-col gap-4 overflow-y-auto flex-1">
          {KEY_GROUPS.map(group => (
            <div key={group.label}>
              <div className="text-[10px] text-accent uppercase tracking-widest mb-2 font-medium">
                {group.label}
              </div>
              {'note' in group && group.note && (
                <div className="text-[10px] text-muted bg-bg border border-border rounded-lg px-3 py-2">
                  {group.note}
                </div>
              )}
              {group.keys.length > 0 && (
                    ...
                    <div key={id} className="bg-bg rounded-xl border border-border p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <a href={url} target="_blank" rel="noopener noreferrer"
                          className="text-[10px] text-accent flex items-center gap-0.5 hover:underline flex-shrink-0">
